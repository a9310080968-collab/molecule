...
const avatarSchema = z.string().max(800_000).refine((value) => value === "" || /^https:\/\//i.test(value) || /^data:image\/(?:png|jpeg|webp);base64,/i.test(value), "Avatar must be an HTTPS URL or a PNG, JPEG, or WebP image.");
...
export function buildServer(config, pool) {
    const app = Fastify({
        logger: {
    ...
    app.post("/api/auth/login", { config: { rateLimit: { max: 5, timeWindow: "1 minute" } } }, async (request, reply) => {
      ...
      LIMIT 1`, [input.email]);
        ...
        const session = await pool.query(`INSERT INTO sessions (user_id, organization_id, token_hash, ip_address, user_agent, expires_at)
       VALUES ($1, $2, $3, $4, $5, $6)
       RETURNING id`, [account.user_id, account.organization_id, tokenHash, request.ip, request.headers["user-agent"] ?? null, expiresAt]);
    ...
    app.patch("/api/users/me", async (request, reply) => {
       ...
       WHERE id = $1
       RETURNING display_name, phone, avatar_url`, [session.user_id, input.name, input.phone, input.avatarUrl || null]);
    ...
    app.post("/api/users/me/password", { config: { rateLimit: { max: 3, timeWindow: "15 minutes" } } }, async (request, reply) => {
        ...
        const passwordResult = await pool.query("SELECT password_hash FROM users WHERE id = $1", [session.user_id]);
...
async function requireSession(request, reply, config, pool) {
...
function cookieOptions(config, expires) {
    return {
        path: "/",
        httpOnly: true,
        sameSite: "strict",
...
async function writeAudit(pool, organizationId, actorUserId, action, entityType, entityId, metadata, ipAddress) {
    await pool.query(`INSERT INTO audit_events (organization_id, actor_user_id, action, entity_type, entity_id, metadata, ip_address)
     VALUES ($1, $2, $3, $4, $5, $6, $7)`, [organizationId, actorUserId, action, entityType, entityId ?? null, metadata, ipAddress]);
}
